<template>
  <div class="bg">
    ...
    >
      ...
      <image
        xlink:href="static/img/login/login-bg.jpg"
        width="1440"
        height="480"
        filter="url(#blur)"
      ></image>
        ...
        <el-tab-pane :label="$t('register')" name="/register" class="login-wrap-title">
          ...
          >
            <el-form-item
              prop="tenantCode"
              v-if="
                sysConfig.sys_login_show_tenant_code &&
                  sysConfig.sys_login_show_tenant_code === 'true'
              "
            >
              <el-input
                :placeholder="$t('tenantCode')"
                v-model="register.form.tenantCode"
                name="tenantCode"
                type="text"
                auto-complete="off"
              />
            ...
            <el-form-item prop="identifier">
              <el-input
                ...
                v-model="register.form.identifier"
                name="identifier"
                type="text"
                auto-complete="off"
              />
            ...
            <el-form-item prop="credential">
              <el-input
                ...
                v-model="register.form.credential"
                name="credential"
                auto-complete="off"
            ...
            <el-form-item prop="code">
              <el-row :span="24">
                <el-col :span="14">
                  <el-input
                    :maxlength="register.code.len"
                    v-model="register.form.code"
                    auto-complete="off"
                    :placeholder="$t('inputValidationCode')"
                    @keyup.enter.native="handleRegister"
                  />
                </el-col>
                <el-col :span="10">
                  <div class="login-code">
                    ...
                    <img
                      v-else
                      :src="register.code.src"
                      :alt="$t('validationCode')"
                      class="login-code-img"
                      @click="refreshRegisterCode"
                    />
                  </div>
            ...
            >
              ...
              <el-form-item prop="code">
                <el-row :span="24">
                  <el-col :span="14">
                    <el-input
                      :maxlength="login.code.len"
                      v-model="login.form.code"
                      auto-complete="off"
                      :placeholder="$t('inputValidationCode')"
                      @keyup.enter.native="handleLogin"
                    />
                  ...
                  <el-col :span="10">
                    <div class="login-code">
                      <span
                        v-if="login.code.type === 'text'"
                        class="login-code-img"
                        @click="refreshLoginCode"
                        >{{ login.code.value }}</span
                      >
                      <img
                        v-else
                        :src="login.code.src"
  ...
  data() {
    ...
    return {
      ...
      login: {
        ...
        rules: {
          identifier: [
            { required: true, trigger: "blur", message: this.$t("validate.inputUsername") }
          ],
          credential: [
      ...
      sms: {
        ...
        rules: {
          phone: [
            {
              required: true,
              message: this.$t("validate.inputPhoneNumber"),
              trigger: "blur",
  ...
  methods: {
    refreshLoginCode() {
      ...
      this.login.code.type === "text"
        ...
        : (this.login.code.src =
            `/sg-user-service/v1/code/${this.login.form.randomStr}?tenantCode=` + getTenantCode());
    },
    refreshRegisterCode() {
      ...
      this.register.code.type === "text"
        ...
        : (this.register.code.src =
            `/sg-user-service/v1/code/${this.register.form.randomStr}?tenantCode=` +
            getTenantCode());
    },
    handleLogin() {
      if (getToken()) {
        ...
        this.$refs.loginForm.validate(valid => {
          if (valid) {
            ...
            // 登录，获取 token
            this.$store
              .dispatch("LoginByUsername", this.login.form)
              .then(() => {
    ...
    handleRegister() {
      this.$refs.registerForm.validate(valid => {
        if (valid) {
          ...
          this.$store
            .dispatch("RegisterByUsername", this.register.form)
            .then(() => {
    ...
    handleSmsLogin() {
      ...
      // 登录，获取 token
      this.$store
        .dispatch("LoginBySocial", this.sms.form)
        .then(() => {
          // 重定向到首页
          this.$router.push("/");
        })
